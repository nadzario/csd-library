import { BookOpen, Download, Eye, FileArchive, FileText, GraduationCap } from 'lucide-react';
...
export function MaterialCard({ material, onPreview }: { material: Material; onPreview: (material: Material) => void }) {
  const canPreview = previewKind(material) !== 'unsupported';
  return (
    <article className="material-card">
      <div className="file-icon"><FileIcon material={material} /></div>
      <div className="material-main">
        <div className="card-meta">
          <span className="format-badge">{formatOf(material)}</span><i />
      ...
      <div className="card-actions">
        {canPreview && <button className="icon-button" onClick={() => onPreview(material)} aria-label="Предпросмотр"><Eye size={18} /></button>}
